...
if ($package) {
  if ($package.version -eq "0.3.3") { Write-Ok "package.json version 0.3.3" } else { Write-FailLine "package.json version attesa 0.3.3, trovata '$($package.version)'" }
...
if ($app -and $app.expo) {
  if ($app.expo.version -eq "0.3.3") { Write-Ok "app.json expo.version 0.3.3" } else { Write-FailLine "app.json expo.version attesa 0.3.3, trovata '$($app.expo.version)'" }
  if ($app.expo.android.versionCode -ge 15) { Write-Ok "Android versionCode >= 16" } else { Write-FailLine "Android versionCode deve essere >= 13" }
  if ($app.expo.ios.buildNumber -eq "16") { Write-Ok "iOS buildNumber 16" } else { Write-FailLine "iOS buildNumber atteso 16" }
  if ($app.expo.extra.baseline -eq "2.0.3") { Write-Ok "extra.baseline 2.0.3" } else { Write-FailLine "extra.baseline atteso 2.0.3" }
