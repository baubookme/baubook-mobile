...
function main() {
  ...
  assert(pkg.baubook && pkg.baubook.baseline === '2.1.1', 'baseline BauBook 2.1.1 mancante in package.json.');
  ...
  if (fs.existsSync(appJsonPath)) {
    ...
    assert(expo.version === '0.4.1', `app.json expo.version attesa 0.4.1, trovata ${expo.version}.`);
    assert(Number(expo.android && expo.android.versionCode) >= 22, 'app.json android.versionCode atteso >= 22.');
    assert(Number(expo.ios && expo.ios.buildNumber) >= 22, 'app.json ios.buildNumber atteso >= 22.');
  }
