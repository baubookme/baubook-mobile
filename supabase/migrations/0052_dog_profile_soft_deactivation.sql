-- BauBook 0.7.4
-- Soft-deactivate dog profiles instead of deleting rows.
...
create index if not exists dogs_owner_active_created_idx
  on public.dogs(owner_id, is_active, created_at);

drop index if exists public.dogs_owner_name_unique;
