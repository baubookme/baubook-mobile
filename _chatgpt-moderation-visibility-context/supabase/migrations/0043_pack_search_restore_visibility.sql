...
-- Restore Dog Friends search/list visibility after block/unblock changes.
-- Important: Dog Friends search deliberately has no strict moderation_status = approved gate.
-- Earlier BauBook migrations 0022/0025 removed that gate because normal beta dog profiles must be searchable.
-- Blocks created by the current user stay visible as is_blocked=true so they can be unblocked.
-- Blocks created by the other user against the current user are still respected.
